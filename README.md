...
Проект загружает переменные окружения из `.env` автоматически (через `python-dotenv`).
...
По умолчанию берётся лист `"Лист2"`. Это можно переопределить при вызове `build_raw` (на уровне кода).
В текущем CLI переопределение листа не поддерживается, используется дефолт.
...
- **XLSX не читается**: укажите корректный `sheet_name` и индексы колонок (переопределяется на уровне функции `build_raw`).

---

## Демо на встроенном примере

В репозитории есть пример входного файла `src/data/Только новые НП.xlsx` и папка с примерами выходных артефактов `src/data_/`.

### Запуск на примере (OpenWebUI)
```bash
USE_OPENWEBUI=1 \
OPENWEBUI_BASE_URL="https://webui.g-309.ru" \
OPENWEBUI_API_KEY="<token>" \
OPENWEBUI_MODEL="gpt-oss:120b" \
python -m src.model process "$(pwd)/src/data/Только новые НП.xlsx" "$(pwd)/src/data_"
```

### Запуск на примере (локальный vLLM)
```bash
USE_OPENWEBUI=0 \
python -m src.model process "$(pwd)/src/data/Только новые НП.xlsx" "$(pwd)/src/data_"
```

После выполнения ожидаются артефакты в `src/data_/`: `raw.csv`, `enriched_Только новые НП.csv`, `search_index/*`, `risk_Только новые НП.csv`.
...
- `src/` — исходники
  ...
  - `data/` — пример входных данных → ПУТЬ: `src/data/`
  - `data_/` — пример выходных/рабочих файлов → ПУТЬ: `src/data_/`
- `tests/` — автотесты
